<template>
    <div id="main-content" class="mx-3 mt-4 w-100 bg-white border">
        ...
        <div class="px-3 py-4 d-flex justify-content-between border-bottom">
            ...
            <div class="me-3">
                <router-link :to="{ name: 'nuevaCompania' }" class="btn btn-info text-white"><i
                        class="fas fa-user-plus fa-fw pr-2"></i> Añadir
                    compañia</router-link>
            </div>
        </div>
        ...
        <table class="table table-responsive">
            ...
            <tbody>
                <tr v-for="company in companies" :key="company.id">
                    ...
                    <td class="text-nowrap">{{ company.slug }}</td>
                    <td class="text-nowrap">{{ company.country }}</td>
                    <td class="text-nowrap">{{ company.status }}</td>
                    <td>
                        <div class="btn-group" role="group" aria-label="Basic example">
                            <router-link :to="{ name:'editarCompania',params:{id:company.id}}" class="btn btn-primary"><i class="fa-solid fa-pen-to-square fa-fw"></i></router-link>
...
export default {
    ...
    methods: {
        async mostrarUsuarios() {
            await axios.get('/api/companies/paginate')
                .then(respuesta => {
                    this.companies = respuesta.data.data;
        ...
        onPageChange(page) {
            ...
            axios.get(`/api/companies/paginate/?page=${page}`)
            .then(respuesta=>{
                this.companies = respuesta.data.data;
            })
        }
