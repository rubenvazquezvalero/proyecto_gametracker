<template>
    ...
    <div id="main-content" class="mx-3 mt-4 w-100 bg-white border">
        ...
        <div class="px-3 py-4 d-flex justify-content-between border-bottom">
            ...
            <div class="me-3">
                <button class="btn btn-info text-white" @click="submitCrearCompania"><i
                        class="fas fa-user-plus fa-fw pr-2"></i> Añadir
                    compañia</button>
            </div>
        </div>
        <form id="game-form" class="m-auto mt-5" action="javascript:void(0)" @submit.prevent="crearCompania"
            method="post">
...
export default {
    ...
    methods: {
        ...
        async crearCompania() {
            ...
            await axios.post('/api/companies', this.companyData)
                .then(respuesta => {
                    console.log(respuesta.data);
                    if (respuesta.status === 200) {
                        this.$toastr.s(`Compañia ${this.companyData.name} creada`);
                        this.companyData = {
                            name: "",
                            slug: "",
                            description: "",
